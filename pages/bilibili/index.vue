  ...
  <view class="container">
    ...
    <view class="header">
      <view class="title-section">
        <text class="logo-text">哔哩哔哩热搜</text>
    ...
    <view class="hot-list" v-if="hotData && hotData.data">
      <view class="hot-item" v-for="(item, index) in hotData.data" :key="index" @click="openBilibiliLink(item.link)">
        <view class="hot-rank">
    ...
    <view class="loading" v-if="loading">
      <text>正在获取哔哩哔哩热搜数据...</text>
    </view>

    <!-- 错误状态 -->
    <view class="error" v-if="error">
      <text>{{ error }}</text>
      <button class="retry-btn" @click="fetchBilibiliHot">重新获取</button>
    </view>
  </view>
</template>
...
const fetchBilibiliHot = async () => {
  ...
  try {
    // 获取API地址（支持自动重试）
    const apiUrl = await getApiUrl('bilibili')
    ...
    console.error('获取哔哩哔哩热搜数据失败:', err)
  } finally {
    loading.value = false
  }
}
...
const openBilibiliLink = (link) => {
  if (link) {
    uni.showModal({
      title: '打开哔哩哔哩',
      content: '是否打开哔哩哔哩查看详情？',
      success: (res) => {
        if (res.confirm) {
          // 复制链接到剪贴板
