  ...
  <view class="container">
    ...
    <view class="header">
      <view class="title-section">
        <text class="logo-text">微博热搜</text>
    ...
    <view class="hot-list" v-if="hotData && hotData.data">
      <view class="hot-item" v-for="(item, index) in hotData.data" :key="index" @click="openWeiboLink(item.link)">
        <view class="hot-rank">
    ...
    <view class="loading" v-if="loading">
      <text>正在获取微博热搜数据...</text>
    </view>

    <!-- 错误状态 -->
    <view class="error" v-if="error">
      <text>{{ error }}</text>
      <button class="retry-btn" @click="fetchWeiboHot">重新获取</button>
    </view>
  </view>
</template>
...
const fetchWeiboHot = async () => {
  ...
  try {
    // 获取API地址（支持自动重试）
    const apiUrl = await getApiUrl('weibo')
